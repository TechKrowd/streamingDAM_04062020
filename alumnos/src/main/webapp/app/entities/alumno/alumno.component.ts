...
export class AlumnoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.alumnoService
        ...
        .subscribe(
          (res: HttpResponse<IAlumno[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInAlumnos(): void {
    this.eventSubscriber = this.eventManager.subscribe('alumnoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAlumno[] | null, headers: HttpHeaders, page: number): void {
